...
class ChromeService(BaseService):
    ...
    def _extract_profile_information(
        self, profile_path: str
    ) -> Optional[ChromeProfileInfo]:
        """Extract profile information from Chrome's Preferences file."""
        self.logger.debug(
            "Starting to extract profile information from: %s", profile_path
        )
        try:
            preferences_file = os.path.join(profile_path, "Preferences")
            if not os.path.exists(preferences_file):
                self.logger.warning(
                    "Preferences file not found at: %s", preferences_file
                )
                return None
            ...
            account_data = (
                extract_jsonpath_value(preferences, "$.account_info[0]") or {}
            )
            ...
            self.logger.debug(
                "Successfully extracted profile information: name=%s, email=%s",
                profile_name,
                email,
            )
    ...
    def _build_chrome_command(self, url: str, profile: ChromeProfile) -> list[str]:
        ...
        self.logger.debug(
            "Building Chrome command for URL: %s, profile: %s", url, profile.id
        )
    ...
    def _validate_and_normalize_url(self, url: str) -> str:
        ...
        if parsed_url.scheme in ("http", "https") and not parsed_url.netloc:
            self.logger.error(
                "Invalid URL: missing domain for %s URL", parsed_url.scheme
            )
    ...
    async def open_url_in_profile(
        self, url: str, profile_id: str
    ) -> ChromeOpenRequestResult:
        """Open a URL in a specific Chrome profile using AsyncCommand."""

        self.logger.info(
            "Starting to open URL in Chrome profile: URL=%s, profile=%s",
            url,
            profile_id,
        )
