...
async def check_docker_health(container_name: str):
    ...
    result = docker_service.health_check_container(container_name)
...
async def get_container_info(container_name: str):
    ...
    result = docker_service.get_container_info(container_name)
    if not result.success:
        raise HTTPException(status_code=400, detail=result.error)
    return result
...
async def list_containers(all_containers: bool = False):
    ...
    result = docker_service.list_containers(all_containers=all_containers)
    if not result.success:
        raise HTTPException(status_code=400, detail=result.error)
    return result
...
async def restart_container(request: ContainerNameRequest):
    ...
    result = docker_service.restart_container(request.container_name)
    if not result.success:
        raise HTTPException(status_code=400, detail=result.error)
    return result
...
async def redeploy_container(request: RedeployRequest):
    ...
    result = docker_service.redeploy_container(request)
    if not result.success:
        raise HTTPException(status_code=400, detail=result.error)
    return result
...
async def get_container_logs(container_name: str, tail_lines: int = 100):
    ...
    result = docker_service.get_container_logs(container_name, tail_lines)
    if not result.success:
        raise HTTPException(status_code=400, detail=result.error)
    return result
...
async def remove_container(container_name: str, force: bool = False):
    ...
    result = docker_service.remove_container(container_name, force)
    if not result.success:
        raise HTTPException(status_code=400, detail=result.error)
    return result
